...
class Blockchain:
    ...
    def add_transaction(self, sender , recevier, amount):
        self.transactions.append({'sender' : sender, 'reciever': recevier, 'amount': amount})
...
app = Flask(__name__)
# Creating an address for the node on Port 1312
...
def mine_block():
    ...
    blockchain.add_transaction(sender= node_address, recevier='Hamza', amount=500)
    block = blockchain.create_block(proof,previous_hash)
    response = {
        'message' : 'Congratulations for mining the block!',
        'index': block['index'],
...
def connect_node():
    ...
    response = {'message':'All nodes are connected, The Oubraneom Blockchain now contains the following nodes: ' ,'total_nodes':list(blockchain.nodes)}
    return jsonify(response), 201
@app.route('/replace_chain', methods=['GET'])
def replace_chain():
    is_chain_replaced = blockchain.replace_chain()
    if is_chain_replaced:
        response = {'message': "The nodes has different chains so the chain was replaced by the longest chains.", 'new_chain': blockchain.chain}
    else:
